...
. Run the `seedu.faculty.MainApp` and try a few commands
...
`Main` has only one class called link:{repoURL}/src/main/java/seedu/faculty/MainApp.java[`MainApp`]. It is responsible for,
...
*API* : link:{repoURL}/src/main/java/seedu/faculty/ui/Ui.java[`Ui.java`]
...
The `UI` component uses JavaFx UI framework. The layout of these UI parts are defined in matching `.fxml` files that are in the `src/main/resources/view` folder. For example, the layout of the link:{repoURL}/src/main/java/seedu/faculty/ui/MainWindow.java[`MainWindow`] is specified in link:{repoURL}/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`]
...
*API* :
link:{repoURL}/src/main/java/seedu/faculty/logic/Logic.java[`Logic.java`]
...
*API* : link:{repoURL}/src/main/java/seedu/faculty/model/Model.java[`Model.java`]
...
*API* : link:{repoURL}/src/main/java/seedu/faculty/storage/Storage.java[`Storage.java`]
...
When the user inputs the email in the CLI, the app fetches the email faculty of the student `index` specified and the subject and body of the email specified. This is converted to JSON, and passed as the `body` of the 'POST' request to Outlook.

The `CommandResult` object that is returned by the email command lets the user know if the email faculty has been successfully send to the student whose index is specified.
...
It follows a similar command flow as that of the faculty book, but it will have it's own separate model and
...
This will simply create an event with the specified fields and store it similar to how the addressbook stores a student.
...
within ConTAct. It follows a similar command flow as that of the faculty book, and the command will uses the model and
...
This will simply specify the attendance field of the Student with the specified attendance and store it similar to how the addressbook stores a student.
...
* `VersionedAddressBook#commit()` -- Saves the current faculty book state in its history.
* `VersionedAddressBook#undo()` -- Restores the previous faculty book state from its history.
* `VersionedAddressBook#redo()` -- Restores a previously undone faculty book state from its history.

These operations are exposed in the `Model` interface as `Model#commitAddressBook()`, `Model#undoAddressBook()` and `Model#redoAddressBook()` respectively.

Given below is an example usage scenario and how the undo/redo mechanism behaves at each step.

Step 1. The user launches the application for the first time. The `VersionedAddressBook` will be initialized with the initial faculty book state, and the `currentStatePointer` pointing to that single faculty book state.

image::UndoRedoStartingStateListDiagram.png[width="800"]

Step 2. The user executes `delete 5` command to delete the 5th student in the faculty book. The `delete` command calls `Model#commitAddressBook()`, causing the modified state of the faculty book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted faculty book state.

image::UndoRedoNewCommand1StateListDiagram.png[width="800"]

Step 3. The user executes `add n/David ...` to add a new student. The `add` command also calls `Model#commitAddressBook()`, causing another modified faculty book state to be saved into the `addressBookStateList`.

image::UndoRedoNewCommand2StateListDiagram.png[width="800"]

[NOTE]
If a command fails its execution, it will not call `Model#commitAddressBook()`, so the faculty book state will not be saved into the `addressBookStateList`.

Step 4. The user now decides that adding the student was a mistake, and decides to undo that action by executing the `undo` command. The `undo` command will call `Model#undoAddressBook()`, which will shift the `currentStatePointer` once to the left, pointing it to the previous faculty book state, and restores the faculty book to that state.

image::UndoRedoExecuteUndoStateListDiagram.png[width="800"]

[NOTE]
If the `currentStatePointer` is at index 0, pointing to the initial faculty book state, then there are no previous faculty book states to restore. The `undo` command uses `Model#canUndoAddressBook()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the undo.

The following sequence diagram shows how the undo operation works:

image::UndoRedoSequenceDiagram.png[width="800"]

The `redo` command does the opposite -- it calls `Model#redoAddressBook()`, which shifts the `currentStatePointer` once to the right, pointing to the previously undone state, and restores the faculty book to that state.

[NOTE]
If the `currentStatePointer` is at index `addressBookStateList.size() - 1`, pointing to the latest faculty book state, then there are no undone faculty book states to restore. The `redo` command uses `Model#canRedoAddressBook()` to check if this is the case. If so, it will return an error to the user rather than attempting to perform the redo.

Step 5. The user then decides to execute the command `list`. Commands that do not modify the faculty book, such as `list`, will usually not call `Model#commitAddressBook()`, `Model#undoAddressBook()` or `Model#redoAddressBook()`. Thus, the `addressBookStateList` remains unchanged.

image::UndoRedoNewCommand3StateListDiagram.png[width="800"]

Step 6. The user executes `clear`, which calls `Model#commitAddressBook()`. Since the `currentStatePointer` is not pointing at the end of the `addressBookStateList`, all faculty book states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
...
* **Alternative 1 (current choice):** Saves the entire faculty book.
...
* **Alternative 1 (current choice):** Use a list to store the history of faculty book states.
...
.. _Unit tests_ that test the individual components. These are in `seedu.faculty.ui` package.
.  *Non-GUI Tests* - These are tests not involving the GUI. They include,
..  _Unit tests_ targeting the lowest level methods/classes. +
e.g. `seedu.faculty.commons.StringUtilTest`
..  _Integration tests_ that are checking the integration of multiple code units (those code units are assumed to be working). +
e.g. `seedu.faculty.storage.StorageManagerTest`
..  Hybrids of unit and integration tests. These test are checking multiple code units as well as how the are connected together. +
e.g. `seedu.faculty.logic.LogicManagerTest`
...
.  Update the version number in link:{repoURL}/src/main/java/seedu/faculty/MainApp.java[`MainApp.java`].
