...
* Date: The value of the event, valid if expressed in this forms:  `DD-MM-YYYY`.
It must also be a value that actually falls within a calendar and between the years 1600 to 9999 (i.e. dates such as 25-13-2000 are invalid).
...
* Description: The value of the event, valid if it is not an empty string.

Currently, the main operation is the `schedule` command, and it is implemented as such:

* `schedule event/EVENT_NAME value/DATE start/START_TIME end/END_TIME descr/DESCRIPTION`
...
`schedule event/Office Christmas Party value/23-12-2018 start/18:00 end/20:00 descr/Party at Scranton Branch`
...
Once scheduled, the list of events will be sorted by chronological order (taking the value and the start time).

The GUI itself will have a calendar component which will display the user's current list of events that they have
scheduled, while adjusting for the current value.
