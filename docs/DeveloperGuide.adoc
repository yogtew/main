...
The main commands supporting this feature is `schedule` and `cancel`, which both interact with the `Calendar` model.
...
The undo/redo feature is also facilitated by a VersionedCalendar, and further details are as described in the undo/redo feature.
