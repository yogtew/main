...
The commands adhere to the following pattern:
 ...
 mark [m/m1] <subcommand> <arguments>`

The arguments `m/m1` may be optional in some cases and default to `Mark.DEFAULT_NAME`

==== Sub-commands
* `find name` -- marks Students matched by name
* `find t/tag...` -- marks Students matched by tags
...
image::MarkCommandClassDiagram.png[]

==== Planned Sub-commands (2.0)
* `index <indexes>` -- marks students by index, separated by space, e.g. `mark index 1 3 4 5`

==== Parser Flowchart

image::MarkCommandParserFlowchart.png[]

==== Execution

image::MarkCommandSeqDiagram.png[]
...
This causes the stored `Student` objects in existing Marks to be outdated, and attempts to use those objects will lead to a crash.

To solve this issue, after every command where student(s) are changed, the stored Marks will be updated as depicted in the activity diagram below.
...
** separate commands into `mark-find`, `mark-show` etc.
** Pros: Easy parsing, easy to understand

* Single command (Current)
** single root command `mark` with subcommands `mark find`, `mark show`
