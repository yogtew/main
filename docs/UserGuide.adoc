...
E.g. `mark m/myMarkName find t/friends` will add the Students tagged with "friends" to a Mark called "myMarkName"
...
==== Displaying Marks
The Students in a Mark can be shown in the GUI with the following command:

`mark [m/MARKTOBESHOWN] show`

==== Interactions with `undo` and `redo`
Due to the way `undo` and `redo` are implemented, Marks that are changed as a result of these commands will not behave as expected, the students that are changed will be removed from the Mark.
