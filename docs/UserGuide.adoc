...
Format: `attendance INDEX_NUMBER at/ATTENDANCE` or `attendance g/MARK_NAME at/ATTENDANCE`

****
* The attendance marking as present is done individually, by using the index number of the student, and the user can individually group students who are absent as 'absent'/'0' and who are present as 'present'/'1'. The user can also collectively group the attendance using the MARK feature. Then, the user can update the attendance of the students who do not follow the majority specifically.
****
...
==== Named Groups
Groups can be named by using the prefix `g/MARKNAME`. If not specified, the name of the created Group will default to "g/default".
...
Format: `group [g/DEST_MARK] <operation> g/OPERANDMARK1 [g/OPERANDMARK2]`
