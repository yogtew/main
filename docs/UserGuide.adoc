...
Adds a student to the faculty book +
...
Shows a list of all students in the faculty book. +
Format: `list`

=== Editing a student : `edit`

Edits an existing student in the faculty book. +
...
Edits the studentNumber number and email faculty of the 1st student to be `91234567` and `johndoe@example.com` respectively.
...
Gathers the email faculty of the student with index 2 for emailing, and sets the subject and body respectively.
...
Deletes the specified student from the faculty book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd student in the faculty book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd student in the faculty book.
* `find Betsy` +
`select 1` +
...
Restores the faculty book to the state before the previous _undoable_ command was executed. +
Format: `undo`

[NOTE]
====
Undoable commands: those commands that modify the faculty book's content (`add`, `delete`, `edit` and `clear`).
====
...
Clears all entries from the faculty book while still maintaining the calendar. +
Format: `clear`
